...
describe('server api rewrites', () => {
  ...
  describe('requests', () => {
    const server = createBackgroundServer({
      command: ['node', path.join(projectRoot, '__e2e__/static-rewrites/express.js')],
      host: (chunk) =>
    ...
    it(`correctly shows the destination contents after a rewrite`, async () => {
      ...
      expect(getIndexText(await rewriteResponse.text())).toEqual(
        getIndexText(await indexResponse.text())
      );
    });
